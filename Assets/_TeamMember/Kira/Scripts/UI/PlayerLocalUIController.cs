...
public class PlayerLocalUIController : NetworkBehaviour {
    ...
    void Update() {
        //スキルの表示状態管理
        if (player.paramater.isCanSkill) {
            skill_State.fillAmount = 1.0f;
            skill_State.color = Color.yellow;
        }
        else {
            skillStateProgress = player.paramater.skillAfterTime / player.paramater.equippedSkills[0].cooldown;
            skill_State.fillAmount = skillStateProgress;
            skill_State.color = Color.white;
        }
        //パッシブの表示状態管理
        if (player.paramater.equippedPassives[0].isPassiveActive || player.paramater.equippedPassives[0].passiveChains >= 1) {
            passiveChains.text = player.paramater.equippedPassives[0].passiveChains.ToString();
        ...
        //現在使用している武器タイプで分岐
        switch (player.paramater.weaponController_main.weaponData.type) {
            case WeaponType.Melee:
                mainWeaponText[(int)TextIndex.Current].text = "∞";
                break;
            case WeaponType.Gun:
                //メインウェポンの現在弾倉数を更新
                mainWeaponText[(int)TextIndex.Current].text = player.paramater.weaponController_main.ammo.ToString();
                break;
            case WeaponType.Magic:
                //所持している武器が魔法であるか確認。
                if (player.paramater.weaponController_main.weaponData is not MainMagicData magicData) {
                    #if UNITY_EDITOR
        ...
        mpText.text = player.paramater.MP.ToString();

        //サブウェポンの現在所持数を更新
        subWeaponText[(int)TextIndex.Current].text = player.paramater.weaponController_sub.currentUses.ToString();

        //パッシブの蓄積数が0だったら空欄にする
        if (player.paramater.equippedPassives[0].passiveChains == 0) passiveChains.text = "";
    }
    ...
    public void LocalUIChanged() {
        //ステータス系の初期化
        hpText.text = player.paramater.HP.ToString();
        ChangeHPUI(player.paramater.maxHP, player.paramater.HP);
        mpText.text = player.paramater.MP.ToString();
        ChangeMPUI(player.paramater.maxMP, player.paramater.MP);


        for (int i = 0; i < skill_Icon.Length; i++) {
            skill_Icon[i].sprite = player.paramater.equippedSkills[0].skillIcon;
        }
        for (int i = 0; i < passive_Icon.Length; i++) {
            passive_Icon[i].sprite = player.paramater.equippedPassives[0].passiveIcon;
        }

        mainWeaponText[(int)TextIndex.WeaponName].text = player.paramater.weaponController_main.weaponData.weaponName;
        //プレイヤーの弾倉が存在すればメインウェポンの弾倉UIを有効化する
        if (player.paramater.weaponController_main.weaponData.type == WeaponType.Gun) {
            mainWeaponText[(int)TextIndex.Partition].text = "/";
            mainWeaponText[(int)TextIndex.Current].text = player.paramater.weaponController_main.ammo.ToString();
            mainWeaponText[(int)TextIndex.Max].text = player.paramater.weaponController_main.weaponData.maxAmmo.ToString();
        }
        else {
            mainWeaponText[(int)TextIndex.Partition].text = "";
            mainWeaponText[(int)TextIndex.Current].text = "";
        ...
        //プレイヤーのサブウェポンUIを反映
        subWeaponText[(int)TextIndex.Current].text = player.paramater.weaponController_sub.currentUses.ToString();
        subWeaponText[(int)TextIndex.Max].text = player.paramater.weaponController_sub.subWeaponData.maxUses.ToString();
        subWeaponText[(int)TextIndex.WeaponName].text = player.paramater.weaponController_sub.subWeaponData.WeaponName;
    }
    ...
    public void StartRotateReloadIcon() {
        if (!reloadIconRotating)
            StartCoroutine(RotateReloadIcon(player.paramater.weaponController_main.weaponData.reloadTime));
    }
