...
public class PlayerListManager : NetworkBehaviour {
    ...
    public void RegisterPlayer(CharacterBase player) {
        if (players.Exists(p => p.name == player.paramater.PlayerName)) return;
        ...
        players.Add(new PlayerInfo(assignedId, player.paramater.PlayerName));
        player.paramater.playerId = assignedId;

        Debug.Log($"[PlayerListManager] 登録: {player.paramater.PlayerName} → Player{assignedId + 1}");
    }
    ...
    public void UnregisterPlayer(CharacterBase player) {
        var target = players.Find(p => p.id == player.paramater.playerId);
        if (target != null) {
    ...
    public List<ResultScoreData> GetResultDataList() {
        ...
        foreach (var p in players) {
            ...
            var chara = FindObjectsOfType<CharacterBase>()
                .FirstOrDefault(c => c.paramater.PlayerName == p.name);

            int team = 0;
            if (chara != null)
                team = chara.paramater.TeamID;
