...
public class ServerManager : NetworkBehaviour {
    ...
    private void JoinRandomTeam(bool _allRandomTeam = false) {
        ...
        foreach (var player in connectPlayer) {
            if (player.GetComponent<GeneralCharacter>().paramater.TeamID != -1)
                continue;
            ...
            if (teams.Capacity != 0) {
                ...
                foreach (var resetTeam in teams) {
                    for (int i = 0, max = resetTeam.teamPlayerList.Count; i < max; i++) {
                        resetTeam.teamPlayerList[i].GetComponent<GeneralCharacter>().paramater.TeamID = -1;
        ...
        for (int i = 0, max = notInTeamPlayer.Count; i < max; i++) {
            ...
            //プレイヤーのチームIDを設定
            player.GetComponent<GeneralCharacter>().paramater.TeamID = teamIndex;
            ChatManager.instance.CmdSendSystemMessage(player.GetComponent<GeneralCharacter>().paramater.PlayerName + " is " + teamIndex + "Team");
        }
